...
import electrolysm.electro.misc.crafting.EnergiserRecipes;
...
public class TileEntityEnergiser extends TileEntity implements IInventory
{
    ...
    private boolean canDoubleSmelt()
    {
        ...
        {
            ItemStack itemstack = EnergiserRecipes.smelting().getDoubleSmeltingResult(this.inventory[0], this.inventory[1]);
            ...
            if (this.inventory[0].stackSize < EnergiserRecipes.smelting().getSlot1ReduceAmount(this.inventory[0]).stackSize)
            {
                return false;
            }

            if (this.inventory[1].stackSize < EnergiserRecipes.smelting().getSlot2ReduceAmount(this.inventory[1]).stackSize)
            {
                return false;
            }
    ...
    public void smeltDoubleItem()
    {
        if (this.canDoubleSmelt())
        {
            ...
            this.inventory[0].stackSize -= EnergiserRecipes.smelting().getSlot1ReduceAmount(this.inventory[0]).stackSize;
            this.inventory[1].stackSize -= EnergiserRecipes.smelting().getSlot2ReduceAmount(this.inventory[1]).stackSize;
