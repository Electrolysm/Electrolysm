...
public class researchPaper extends Item {
    ...
    public void addInformation(ItemStack stack, EntityPlayer player, List list, boolean par4)
    {
        if(stack.stackTagCompound != null) {
            String data = stack.stackTagCompound.getString("researchData");
            Research research = ResearchRegistry.getResearchFromString(data);
            String researchMessage = WordUtils.capitalize(research.getName().replace("_", " "));

            list.add(researchMessage);
        }
        else
        {
