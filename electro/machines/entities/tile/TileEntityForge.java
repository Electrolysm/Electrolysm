...
public class TileEntityForge extends TileEntity implements IInventory {
	...
	public void updateEntity()
	{
		...
		if (!this.worldObj.isRemote)
		{
			...
			if (flag1)
			{
				...
				Forge.updateFurnaceBlockState(this.furnaceBurnTime > 0, this.worldObj, this.xCoord, this.yCoord, this.zCoord);
			}
		}
	}
	...
	private boolean canDoubleSmelt()
	{
		...
		{
			ItemStack itemstack = ForgeRecipes.smelting().getDoubleSmeltingResult(this.inventory[0], this.inventory[1]);

			if (itemstack == null) return false;
			if (this.inventory[0].stackSize < ForgeRecipes.smelting().getSlot1ReduceAmount(this.inventory[0]).stackSize) return false;
			if (this.inventory[1].stackSize < ForgeRecipes.smelting().getSlot2ReduceAmount(this.inventory[1]).stackSize) return false;
	...
	public void smeltDoubleItem()
	{
		if (this.canDoubleSmelt())
		{
			...
			this.inventory[0].stackSize -= ForgeRecipes.smelting().getSlot1ReduceAmount(this.inventory[0]).stackSize;
			this.inventory[1].stackSize -= ForgeRecipes.smelting().getSlot2ReduceAmount(this.inventory[1]).stackSize;
