...
public class TileEntityCrusher extends TileEntity implements IInventory {
	...
	private boolean canDoubleSmelt()
	{
		...
		{
			ItemStack itemstack = CrusherRecipes.smelting().getCrushingResult(this.inventory[0]);

			if (itemstack == null) return false;
			if (this.inventory[0].stackSize < CrusherRecipes.smelting().getSlot1ReduceAmount(this.inventory[0]).stackSize) return false;
			if (this.inventory[1].stackSize < CrusherRecipes.smelting().getSlot2ReduceAmount(this.inventory[1]).stackSize) return false;
	...
	public void smeltDoubleItem()
	{
		if (this.canDoubleSmelt())
		{
			...
			this.inventory[0].stackSize -= CrusherRecipes.smelting().getSlot1ReduceAmount(this.inventory[0]).stackSize;
			this.inventory[1].stackSize -= CrusherRecipes.smelting().getSlot2ReduceAmount(this.inventory[1]).stackSize;
