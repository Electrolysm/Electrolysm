...
public class TileEntityEnergiser extends TileEntity implements IInventory {
	...
	private boolean canDoubleSmelt()
	{
		...
		else
		{
			...
			if (this.inventory[0].stackSize < EnergiserRecipes.smelting().getSlot1ReduceAmount(this.inventory[0]).stackSize) return false;
			if (this.inventory[1].stackSize < EnergiserRecipes.smelting().getSlot2ReduceAmount(this.inventory[1]).stackSize) return false;
